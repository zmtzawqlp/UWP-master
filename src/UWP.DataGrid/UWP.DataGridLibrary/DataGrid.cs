    ...
    public partial class DataGrid : Control
    {
        ...
        private void InitializeContentGrid()
        {
            ...
            if (OuterScrollViewer != null && (OuterScrollViewerVerticalScrollEnable || OuterScrollViewerHorizontalScrollEnable))
            {
                ...
                {
                    _contentGrid.ManipulationMode = ManipulationModes.System;
                }
                else if (OuterScrollViewerVerticalScrollEnable)
                {
                    _contentGrid.ManipulationMode = ManipulationModes.TranslateX | ManipulationModes.System | ManipulationModes.TranslateInertia;
                }
                else if (OuterScrollViewerHorizontalScrollEnable)
                {
                    _contentGrid.ManipulationMode = ManipulationModes.TranslateY | ManipulationModes.System | ManipulationModes.TranslateInertia;
                }
            }
            else
            {
        ...
        private void HanldeOuterScrollViewerWithISupportIncrementalLoading()
        {
            if (OuterScrollViewer != null && OuterScrollViewer.VerticalScrollMode != ScrollMode.Disabled && _view != null)
            {
                DispatcherTimer a = new DispatcherTimer();
                a.Interval = new TimeSpan(100);
                a.Tick += (s, e) =>
                {
                    if (!HasMoreItems())
                    {
        ...
        private void HandleCellAndColumnScrollPosition(Point value, Size sz, double totalRowsSize, double maxH)
        {
            double maxV;
            var scrollPosition = value;
            scrollPosition.Y += HeaderMeasureHeight;

            maxV = totalRowsSize + _columnHeaderPanel.DesiredSize.Height - sz.Height;

            scrollPosition.X = Math.Max(-maxH, Math.Min(scrollPosition.X, 0));
            scrollPosition.Y = Math.Max(-maxV, Math.Min(scrollPosition.Y, 0));

            if (scrollPosition != _cellPanel.ScrollPosition)
            {
                _cellPanel.ScrollPosition = scrollPosition;
                _columnHeaderPanel.ScrollPosition = new Point(scrollPosition.X, 0);
        ...
        private bool HasMoreItems()
        {
            if (_view.HasMoreItems)
            {
                if (!_isLoadingMoreItems)
                {
                    ...
                    if (updateCount == 0 && OuterScrollViewer != null)
                    {
                        updateCount = (uint)((OuterScrollViewer.ActualHeight - (_columnHeaderPanel.ActualHeight + HeaderActualHeight + FooterActualHeight)) / Rows.DefaultSize);
                    }
